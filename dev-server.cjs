...
async function sendMail({ to, subject, html }) {
  ...
  const fromEmail = process.env.SMTP_FROM || process.env.SMTP_USER || 'noreply@gifted.ng'
  try {
...
async function handleRequest(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      const { data: { user: authUser }, error: authErr } = await anonSupabase.auth.getUser(jwt)
      if (authErr || !authUser) {
        res.statusCode = 401
      ...
      if (action === 'approve') {
        ...
        if (isExistingUser) {
          ...
          emailSent = await sendMail({
            ...
            html: `<!DOCTYPE html><html><body style="font-family:-apple-system,sans-serif;max-width:600px;margin:0 auto;padding:24px">
              <h1 style="color:#7c3aed">Welcome Back!</h1>
              <p>Hi ${escapeHtml(name || email)},</p>
              <p>Your Gifted Partner application has been <strong>approved</strong>. You already have an account.</p>
              <p><a href="${escapeHtml(resetUrl)}" style="display:inline-block;padding:12px 24px;background:#7c3aed;color:#fff;text-decoration:none;border-radius:8px">Set Your Password</a></p>
              <p style="color:#666;margin-top:24px">Referral Code: <strong>${escapeHtml(referral_code)}</strong></p>
            </body></html>`,
          })
        } else {
          emailSent = await sendMail({
